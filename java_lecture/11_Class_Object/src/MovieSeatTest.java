...
public class MovieSeatTest {
	...
	// 클래스 메소드 선언
	/*
	 * 메소드명 : reserve 기 능 : 예약이 안 되어 있으면 customer로 예약 처리, 이미 예약돼 있으면 "이미 예약된 좌석입니다."
	 * 출력
	 */
	void reserve(String customer) {
		
		 if ( !isReserved ) { // 아직 예약되지 않은 좌석인지 검사
			 isReserved = true; // 예약 상태로 변경
			 reservedBy = customer; // 매개변수로 받은 고객이름을 예약자로 저장
			 // 예약 완료 메세지 출력
			 System.out.println(seatNumber + " 좌석, " + customer + "님 예약 완료");
		 } else { // 이미 예약된 좌석인지 검사
			 System.out.println(seatNumber + " 좌석은 이미 예약된 좌석입니다."); // 예약 실패 안내 메세지 출력
		 }
	}

	/*
	 * 메소드명 : cancel 기 능 : 예약을 취소하고 예약자 정보를 지운다
	 */
	void cancel() {
		isReserved = false;
		reservedBy = "";
	...
	/*
	 * 메소드명 : printSeat 기 능 : 좌석번호, 예약여부, 예약자를 한 줄로 출력한다
	 */
	void printSeat() {
		System.out.println("좌석번호: " + seatNumber + ", 예약여부: " + (isReserved ? "예약 불가" : "예약 가능") + ", 예약자: " + reservedBy);
	}
