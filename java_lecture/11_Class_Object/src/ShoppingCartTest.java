...
public class ShoppingCartTest {
	...
	// 클래스 메소드 선언
	/*
	 * 메소드명 : addQuantity 기 능 : quantity에 n을 더하고 변경된 수량을 출력한다
	 */
	void addQuantity(int n) {
		quantity += n; // 현재 수량에 추가할 개수 n을 더함
		
		System.out.println(itemName + "의 수량이 " + quantity + "개로 변경되었습니다."); // 변경된 수량 출력
	}

	/*
	 * 메소드명 : calcTotal 기 능 : price * quantity 값을 반환한다
	 */
	int calcTotal() {
	...
	/*
	 * 메소드명 : printCart 기 능 : calcTotal()을 호출해 상품명, 수량, 합계를 출력한다
	 */
	void printCart() {
		System.out.println( "상품명: " + itemName + ", 수량: " + quantity + "개, 합계: " + calcTotal() + "원");
	}
