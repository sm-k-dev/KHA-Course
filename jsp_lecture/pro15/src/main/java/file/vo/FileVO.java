...
/*
	참고.
		VO (Value Object) 클래스
		DB의 테이블에서 조회된 한 줄 (하나의 행 = 레코드)의 정보를 저장해 놓고 관리하는 클래스
		
		실제 File이라는 이름의 데이터베이스 내부에 만들어져 있는 file 테이블의 모습
		
			열 명			filename	filerealname	downloadcount
			조회데이터		보고서.hwp	보고서.hwp			0
			조회데이터		보고서.hwp	보고서1.hwp			3
		
		위 표에서 한 줄 (한 행)의 저오를 저장하는 것이 FileVO객체 1개 이다.
		따라서 조회된 레코드가 2줄 이면 FileVO객체도 2개가 생성
*/
public class FileVO {
	
	/*
		1. 조회된 한 줄의 레코드 정보를 저장할 변수 선언
		private로 선언하는 이유: 외부 클래스에서 직접 변수값을 바꾸지 못 하게 막고
							반드시 아래의 setter 메소드를 통해서만 값을 저장하게 하기 위해
	*/
	...
	/*
		2. 기본 생성자
			JSP 화면에서 EL( ${ vo.file } ) 이 이 객체를 다룰 때 반드시 필요하다.
			아래 3 의 생성자를 만들면 기본생성자가 자동으로 만들어지지 않으므로 개발자가 직접 선언해야 한다.
	*/
	public FileVO() {}
	
	/*
		3. 조회된 레코드 한줄의 정보 3개를 한번에 매개변수로 전달 받아 위 변수에 저장하는 생성자
			DAO에서 ResultSet으로 조회한 값을 한 줄로 얻어 FileVO객체를 만들 때 사용된다.
	*/
	...
	*/
	...
	//	DAO에서 조회한 값을 FileVO객체의 각 변수에 하나씩 채워 넣을 때 사용
