...
public class MemberServlet extends HttpServlet{
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 어떤 요청(회원추가, 회원삭제, 회원수정)인지 판단할 조건값 얻기
		...
		// 회원추가/삭제/수정처리 후 전체회원조회 -> 화면담당 ViewServlet 서블릿으로 포워딩
	...
	private void addMember(HttpServletRequest request) {
		...
		MemberVO vo = new MemberVO( request.getParameter("id"), request.getParameter("pwd"), request.getParameter("name"), request.getParameter("email"));
		
		// MemberDAO에 INSERT 요청 -> 반환값: 성공 1 / 실패 0
		System.out.println("회원가입 결과: " + memberDAO.addMember(vo) );
	}
