...
public class MemberVO { // <- t_member 테이블의  하나의 레코드(회원 한사람의 정보)가 저장되는 클래스 
    ...
    // 회원의 주소정보와 우편번호 정보를 가지고 있는 Address 클래스의 객체의 주소를 저장할 변수
    private	Address	address; // <- new Address(); (MemberVO는 Address 클래스를 포함하고 있다, has on 관계)
	...
	*/   
	...
	// address의 변수 값을 외부 클래스에서 변경해서 저장시킬 setter 메소드 정의
	public void setAddress( Address address ) {
		this.address = address;
	}
	
	// address 변수에 저장된 new Address(); 객체 주소를 외부클래스에서 얻기 위해 호출하는 getter 메소드 정의
