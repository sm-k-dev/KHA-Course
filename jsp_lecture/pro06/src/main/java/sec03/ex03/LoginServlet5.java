...
/*
login5.html(아이디와 비밀번호를 입력해서 로그인버튼을 눌러 서블릿을 요청하는 화면)에서 
로그인 버튼을 눌러  http://localhost:8181/pro06/login5 
요청주소를 톰캣 서버에 전달하여 요청하면
요청을 받는 LoginServlet5클래스 입니다.
...
//doGet, doPost 메소드 모두 오버라이딩 후 각 메소드 내부에서 doHande메소드를 다시 호출해서 요청 받고 응답처리 하자
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		System.out.println("login5.html로 부터 GET전송 요청 방법으로 로그인 요청 받았다. doGet() 메소드 내부 코드이다.");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("login5.thml로 부터 POST전송 요청 방법으로 로그인 요청 받았다. doPost() 메소드 내부 코드이다.");
		doHandle(request, response);
	}
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		String address = request.getParameter("user_address");	// <input type="hidden" name="user_address" value="서울시 성북구">
																// 에 설정해서 요청한 "서울시 성북구" 주소 문자열로 얻기
		...
		*/
		...
		String data =	"<html>";
			   ...
			   data +=			"요청시 hidden으로 받은 주소: " + address + "<br>" ;
			   data +=		"</body>";
			   data +=	"</html>";
