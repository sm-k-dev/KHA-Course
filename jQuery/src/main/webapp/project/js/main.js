...
//-----------------------------------------------------------
   ...
   */
	...
	/*
	 	3. autoPlay 함수 정의
	 		역할: 4초 간격으로 호출되어 [1] ~ [4] 순차적으로 강제로 클릭되어 자동으로 배너 이미지영역 <dd> 요소를 보이도록 하는 기능
	*/
	let btnNum = 0;
	
	function autoPlay() {
		btnNum++; // 일정한 시간간격으로 실행문이 실행될때 마다 btnNum 의 값이 1씩 증가 되게
		
		// 이때 btnNum 변수의 값이 4 이상 되는 순간 0으로 변경해서 [1] <a>를 강제 클릭할 수 있도록 btnNum 변수값 0으로 변경
		if (btnNum >= 4) btnNum = 0;
		
		// 4초 간격으로 [1][2][3][4] 를 순차적으로 강제 클릭되게 하자.
		$("#roll_banner_wrap dt a").eq(btnNum).trigger("click");
		
		// 4초 간격으로 재귀함수 호출(자기 자신의 autoPaly() 함수 재호출)
		auto1 = window.setTimeout( autoPlay, 4000 );
	}
	...
	$(".playBtn").on("click",function(){
		...
		/* 
			사이트 방문자가 재생 버튼을 여러번 클릭하게 되면
			auto1 변수에 있는 setTimeout() 메소드들이 쌓여 있어 문제가 될 수 있음
			setTimeout() 메소드를 clearTimeout() 메소드로 제거한 후 setTimeout() 메소드 호출구문의 실행을 해야한다.
		*/
		window.clearTimeout(auto1); // auto1 변수에 저장된 setTimeout() 메소드 제거
		auto1 = window.setTimeout(autoPlay, 1000); // 1초 지연 후 autoPaly 함수 호출
		
		// 클릭한 재생 버튼 img 태그의 src 속성 주소 값을 분홍색 이미지 주소로 변경해서 보여줌
		$("img", this).attr("src", "images/pop_btn_play_on.gif");
		
		// 클릭하지 않은 정지 버튼 img 태그의 src 속성 주소값을 회색 이미지 주소로 변경해서 보여줌
		$(".stopBtn img").attr("src", "images/pop_btn_stop_off.gif");
		
		return false;
	});
	
	// 5. 정지버튼 <a> 영역을 클릭했을 때 일정한 시간간격으로 autoPlay함수가 실행되는 것을 제거하고
	//		정지 버튼 이미지를 분홍색 이미지로 변경해서 보여주자
	$(".stopBtn").on("click", function(){
		
		// auto1 변수에 저장된 setTimeout(autoPlay, 3000) 메소드 호출 구문 제거
		window.clearTimeout(auto1);
		
		// 정지 버튼 회색 이미지 > 정지 버튼 분홍색 이미지로 변경
		$("img", this).attr("src", "images/pop_btn_stop_on.gif");
		
		// 재생 버튼 분홍색 이미지를 > 재생 버튼 회색 이미지로 변경
 ...
 //-----------------------------------------------------------
   ...
   */
	...
	// [공지사항] 탭의 <a> 요소를 선택해서 onTab 변수에 저장
	let onTab = $("#tabmenu dt a:first");
	// 탭 버튼의 <a> 태그들 (공지사항, 질문과 답변, 저자문의) 을 한번에 모두 선택
	// mouseover(마우스를 올리는 동작), focus(포커스를 생성하는 동장), click(클릭하는 동작) 들을 다중 등록
	$("#tabmenu dt a").on("mouseover focus click", function(){
		// 현재 노출(:visible)되어 있는 <dd> 태그를 선택하여
		// 게시물 리스트영역 <dd>들을 숨김(.hide() 메소드)
		$("#tabmenu dd:visible").hide();
		
		// onTab 변수에 저장된 [공지사항] <a> 태그 영역의 하위 <img>를 선택해
		// 빨간색 이미지에서 회색(이벤트가 발생하지 않은 이미지 색)으로 변경
		$("img", onTab).attr("src", $("img", onTab).attr("src").replace("over.gif", "out.gif") );
		
		//등록한 3개의 이벤트 (mouseover, focus, click)중 하나의 이벤트가 발생한~ <a>를 선택해서 
		//선택한 <a>의 부모요소<dt>를 두번쨰로 선택하고 
		//<dt>다음에 작성된 <dd>태그영역을 최종 선택하여 show()메소드를 사용해 화면에 보여주자
		$(this).parent().next().show();
		
		//등록한 3개의 이벤트 (mouseover, focus, click)중 하나의 이벤트가 발생한~ <a>를 부모요소로 먼저 선택해서 
		//부모<a>의 하위 <img>요소를 최종선택해서 src속성의 주소값을 회색이미지 주소에서 빨간색이미지 주소로 변경
		$("img",this).attr("src", $("img",this).attr("src").replace("out.gif","over.gif") );
		
		//등록한 3개의 이벤트 (mouseover, focus, click)중 하나의 이벤트가 발생한~ <a>를 요소를 선택해서
		//onTab변수에 저장 
		onTab = $(this);
...
*/	
  ...
  */
	 /*
	 	 팝업창 연동 
	 	 설명 :  저장된 "pop"에 쿠키값을 검사하여 만일 쿠기가 브라우저에 저장되어 있지 않으면?
	         숨겨져 있던 팝업창을 브라우저화면에 보여주자.
	 			그리고 사이트 이용자가 [하루동안 이창열지않기] 를 클릭했을때
	 		쿠기가 생성되어 웹브라우저에 쿠키저장소에 보관되어  하루동안 팝업창이 열리지 않도록 하자.
	  */  
	 //조건 : 만약 "pop"쿠키명과 함께 "no"쿠키값이 웹브라우저에 저장되어 있지 않으면?
	 //		숨겨져 있던 팝업창을 보이게 만들자.
	 if(  $.cookie("pop")  != "no" ){
	 ...
	 //팝업창 영역 id="pop_wrap"에  마우스 포인터를 올리면  마우스 포인터 모양을 십자가 모양으로 변경해서 보여주자
	 //그리고 JQuery UI 플러그인에서 제공하는  draggable() 메소드를 사용하여 마우스로 드래그를 했을때 자연스럽게 이동되게 보여주자
	 $("#pop_wrap").css("cursor","move").draggable();
	 		
	 //팝업창 <p id="pop_wrap">영역의 하위 후손 <area> 두쌍을 배열에 담아 선택해 오되
	 //배열의 0 index위치 칸에 저장된 첫번째 <area>[창닫기] 요소를 최종 선택 해서 "click" 이벤트 등록 후 처리 
	 $("#pop_wrap area:eq(0)").on("click", function(){
	 	
	 	//팝업창 영역을 선택해  점점 투명해 지면서 사라지게 하자.
	 ...
	 //팝업창 <p id="pop_wrap">영역의 하위 후손 <area> 두쌍을 배열에 담아 선택해 오되
	 //배열의 1 index위치 칸에 저장된 두번째 <area>[하루동안 이창 열지 않기] 요소를 최종 선택 해서 "click" 이벤트 등록 후 처리 
	 $("#pop_wrap area:eq(1)").on("click", function(){
	 	
	 	//웹브라우저 쿠키저장소에 보관될  쿠키 한쌍을 생성해서 보관해 둡니다.
	 	//쿠키명 : "pop"  , 쿠키값 : "no" 가 한쌍으로  오늘로부터 하루동안 보관되게 하자
	 	$.cookie("pop","no",{expires:1});
