...
컴퓨터 공학
    - 단순히 '코딩 기술'을 배우는 것이 아니라, 복잡한 문제를 추상화하여 컴퓨터로 해결하는 사고방식을 배우는 것
    - 알고리즘적 / 계산적 사고: 복잡한 문제를 단순한 단계(Step)들의 조합으로 나누고 전환하는 능력
    - 데이터 기반 사고: 데이터 분포와 인사이트를 바탕으로 문제의 본질을 파악하는 능력
    - 엔지니어링 매니저 역할: 엔지니어링 경험과 코딩 감각 없이는 AI에게 정확한 업무를 위임하고 결과를 판별하기 어렵다.

    - 문제 풀이에서 정의와 비평으로
        미니멀한 제약만 으로 스스로 문제를 정의 설계하는 훈련
        타인의 코드나 디자인을 비평(Critic)하여 좋은 솔루션인지 판별하는 안목
    - 기발자 (기획자 + 개발자)
        문제의 니즈와 도메인 지식을 이해하여 어떤 문제를 풀것인가를 정의하고 끝가지 구현해내는 종합 역량
    
    풀스택 빌더(Full-stack Builder): 해외 빅테크(실리콘밸리, 링크드인 등) 및 일부 국내 혁신 기업에서는 단순 엔지니어나 디자이너 대신 AI 툴을 활용해 기획부터 문제 정의, 설계, 개발, 출시까지 전체 과정을 주도적으로 이끌어가는 사람을 뽑기 시작했습니다

    빌더에게 가장 중요한 핵심 역량
        도메인 지식 (Domain Knowledge)
            코딩과 디자인 기술은 AI가 쉽게 구현해 주지만, "사용자가 왜 이걸 쓰는가?", "이 도메인의 실제 현장 문제점은 무엇인가?"를 판단하는 것은 오직 인간만이 할 수 있습니다 [03:25].
        AI 활용 능력 (바이브 코딩 등)
            AI 프롬프팅 및 바이브 코딩(Vibe Coding)을 통해 생각을 빠르게 프로덕트로 만들어내는 실전 도구 활용력입니다.
        빌더 마인드셋 (Owner Mindset)
            "내 업무는 여기까지"라는 한계를 두지 않고, 문제 정의부터 서비스 출시까지 전 프로세스를 오너십 있게 이끌어가는 태도입니다.

        앞으로의 공부 및 취업 전략
            기본 원리의 이해 필수: 
                AI가 다 해주니 공부가 필요 없는 것이 아니라, AI의 결과물을 검증하고 완성도를 높이기 위해서는 오히려 소프트웨어 개발 원리나 기본 개념을 폭넓게 이해해야 합니다.
            이력서/포트폴리오의 변화: 
                단순히 '어떤 언어/기술 스택을 쓸 줄 안다'가 아니라, "AI를 활용해 현장의 문제를 어떻게 정의하고 실제로 해결(출시)해 보았는가"에 대한 경험 중심으로 채용 패러다임이 전환되고 있습니다.

    개발 패러다임의 변화: 직원에서 CEO 마인드로
        구시대적 바이브 코딩: 
            기획, 설계, UI 테스트까지 사람이 전부 다 하고, 단순 코드 구현만 AI(채팅)에 맡기는 방식은 AI 능력의 10%만 활용하는 것입니다.
        CEO 마인드셋: 
            이제 개발자는 회사의 CEO처럼 전체 방향과 단순 요청만 제시하고, 기획·구현·테스트 등 각 역할별 전문가 역할을 하는 '서브에이전트(Sub-agent)'를 지휘해야 개발 생산성이 비약적으로 상승합니다

------------------------------------------------------------------------------

역발상 학습
    목적/문제 해결부터 시작 -> 돌려보고 막히는 지점만 즉시 파고 들기
    지식의 빈틈 채우기 라고 부르며 내가 무엇을 모르는지 파악하고 필요한 기초를 AI로 빠르게 메꾸어 학습 속도를 획기적으로 줄인다
    
    AI에게 일만 전부 맡기고 결과물만 받아 쓰는 '바이브 코더'가 아니라 'AI 에게 시키되, 생성된 코드는 한 줄 한 줄 내가 직접 다 읽는다' 라는 원칙 고수
        - 지름길을 쓰되, 기초를 건너뛰는 지름길이 아니라 기초를 빠르게 이해하기 위한 지름길로 AI 활용

    AI에게 단순히 만들어줘 하고 끝내는 것이 아니라 이게 왜 이렇게 되는지, 12살 어린이에게 설명하듯 중간 과정을 단계별로 알려달라고 한다.

    [AI 질문 3가지 원칙]
        직접적이고 구체적으로 요청할 것
        결과만 주지 말고, 도달 과정(중간 과정)을 전부 보여줄 것
        12살 어린이에게 설명하듯 쉽게 풀어 설명해 달라고 할 것 (아하! 하고 이해되는 순간을 하루에 100번 이상 만들기)

------------------------------------------------------------------------------

비전공자의 AI 업계 생존 전략
    코딩보다 중요한 것은 '나에 대한 이해'
    자기 객관화와 강점 발견
    나만의 아침반 - 컴퓨터와 경쟁하지 않고 강점에 힘을 싣는다

AI 시대의 핵심: '기술'보다 '문제 정의'
    AI 시대를 개척하는 사람들은 '기술 전문가'가 아닌 '문제 전문가'

Ai 생태계와 '나만의 중심'
    AI는 밀키트다 - 전문가가 아니라 누구나 자신이 잘 아는 영역을 확장 할 수 있다
    도구로서의 AI - AI는 사람이 진짜 집중해야 할 본질적인 일을 돕는 도구일 뿐이다
    나만의 답을 찾는 태도 - 내가 무엇을 좋아하고, 무엇을 잘하며, 내 영역에서 AI를 어떻게 활용할 것인가에 대한 자기만의 기준과 중심을 지킨다
