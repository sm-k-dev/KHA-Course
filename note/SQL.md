...
JOIN
    INNER JOIN
        양쪽 테이블 모두에 매칭되는 데이터가 있는 행만 결합하여 반환합니다.
        가장 많이 쓰이는 기본 조인입니다. INNER를 생략하고 JOIN만 써도 똑같이 동작합니다.

    LEFT [OUTER] JOIN
        왼쪽 테이블의 모든 데이터와, 오른쪽 테이블에서 매칭되는 데이터를 반환합니다.
        오른쪽 테이블에 매칭되는 값이 없으면 NULL로 채워집니다. 
        OUTER 키워드는 생략 가능합니다.

    RIGHT [OUTER] JOIN
        오른쪽 테이블의 모든 데이터와, 왼쪽 테이블에서 매칭되는 데이터를 반환합니다.
        왼쪽 테이블에 매칭되는 값이 없으면 NULL로 채워집니다. LEFT JOIN과 방향만 반대입니다.

    FULL [OUTER] JOIN
        양쪽 테이블의 모든 데이터를 반환합니다.
        매칭되는 데이터는 결합하고, 한쪽에만 있는 데이터는 반대쪽을 NULL로 채워 가져옵니다. 
        (※ MySQL 등 일부 DBMS에서는 지원하지 않아 UNION을 사용해 구현해야 합니다.)

    CROSS JOIN
        두 테이블의 모든 행을 상호 조합하여 가능한 모든 쌍(카테시안 곱)을 반환합니다.
        조인 조건(ON)이 필요 없으며, 결과 행의 수는 A 테이블 행 수 × B 테이블 행 수가 됩니다.

    SELF JOIN
        하나의 테이블을 자기 자신과 조인하는 기법입니다.
        별도의 키워드가 있는 것이 아니라 INNER JOIN이나 LEFT JOIN을 활용하되, 같은 테이블에 서로 다른 별칭(Alias)을 부여하여 사용합니다. 
        (예: 직원 테이블에서 직원의 매니저 이름을 찾을 때 사용)